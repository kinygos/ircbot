...
@helpers.commands('cowsay')
def urb(bot, conn, event):
    message = event.arguments()[0].split(' ', 1)[1].strip()
    cowsay_lines = cowsay(message)
    for line in cowsay_lines.strip().split('\n'):
        conn.privmsg(event.target(), line.rstrip())
